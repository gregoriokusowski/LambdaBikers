(ns bikers.handler
  (:require [compojure.core :refer [defroutes routes]]
            [bikers.routes.home :refer [home-routes]]
            [bikers.middleware
              :refer [development-middleware production-middleware]]
            [bikers.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "bikers.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (cronj/start! session/cleanup-job)
  (timbre/info "\n-=[ bikers started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "bikers is shutting down...")
  (stop-nrepl)
